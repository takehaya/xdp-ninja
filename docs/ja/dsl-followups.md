...
### B-3. parser-block TLV walk への TCP options 移行

**Stage 3 PR-3-1〜4 で landed**:
- `pkt.lookahead<bit<8>>()` を select key に取れる (kind-byte dispatch)
- `pkt.advance(((bit<32>)pkt.lookahead<bit<16>>()[7:0]) << 3)` (template B) で unknown option length-skip
- `pkt.advance(8)` (template C) で NOP padding skip
- `TCP_PARSER_MAX_DEPTH` const で bpf_loop max_iter 指定
- multi-state self-loop codegen (`emitMultiStateSelfLoop` + callback) — TLV walk shape 検出と sibling 吸収
- tcp.p4 を 7-state parser block に migration (parse_options → parse_{nop,mss,ws,sack_perm,ts,unknown_opt} → parse_options)
- 4 kernel matrix (6.1 / 6.6 / 6.12 / 6.18) で BPF verifier 全 pass

**追加 cleanup landed**:
- `OPT_TERMINATOR_KIND` / `PADDING_KIND` / `LENGTH_BYTE_OFF` の triple は RFC universal (TCP RFC 9293 / IPv4 RFC 791 で 0/1/1) のため loader に default を持たせて tcp.p4 から削除。partial declaration は引き続き typo として reject (all-or-nothing)

**残スコープ (deeper architectural cleanup)**:
- `pkg/kunai/codegen/where.go::genOptionLookupLoad` を AuxLayout 経由に書き換える: 現状 where 句の option 参照は packet を where-time に再 walk しているが、parser-machine sibling extract 時に option offset を per-aux スロットに記録し、where-time access は slot から読むようにする
- 必要な infra: AuxLayout に DynamicOffsetSlot を追加 / 多 state callback の sibling extract 前に R3 を slot に store / layer entry で sentinel 値で zero-init / where codegen で sentinel を「option なし → reject」として扱う / resolver で AuxRef.Option を消して通常 aux path に乗せる
- 完了後 `genOptionLookupLoad` + `OptionLookup` IR + OPT_<NAME>_KIND/SIZE consts は削除可
- `docs/ja/dsl-internals.md` §6.5 Mechanism 7 の doc 更新 (新 multi-state codegen + AuxLayout 経由 option access)

**工数**: 1-2 日 (slot allocation + codegen 拡張 + resolver 切替 + dead code 削除)。
