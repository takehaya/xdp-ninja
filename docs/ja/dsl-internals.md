...
8. **`where.go`** — where 節 (or / and / not / arith / action atom / bool atom / bool eq / quantifier any-all)。**het-alt 後の field addressing** (`layerAnchorFor` で abs / slot anchor を出し分け、PR-A/B 由来)。F4 Int<128> arith pipeline (`genArithCompare128` / `genArith128FieldOpConst` / `genArith128FieldOpField` / `genArithField128Load`、ABI-clean な stack-bridged carry/borrow)、F6 bitwise ALU 統合、F10 BoolEq の `genConditionAsBool`、F13 slice の `emitSliceShiftMask`、TCP/IPv4 option-walk emitter (`genOptionLookupLoad`、anchor 経由で abs / slot 両対応)
...
- kunai 占有: arith spill `[-56..-88]` (5 slot — F4 の `field op field` で +1 拡張済)、bpf_loop ctx `[-128..-104]` (4 slot)、per-layer entry slot `[-160..-160-8N]` (`whereLayerEntrySlot`、PR-A/B 由来、het-alt 後の where/capture/option-walk が runtime offset を読むのに使用、`whereLayerEntrySlotCap = 12` 上限 = 96 byte)
...
**実装上の対応**: 上の parser block は `pkt.lookahead<bit<8>>()` を select 鍵に取っているが、これは p4lite が**サポートしない** (P4-16 の `lookahead` は subset から除外、§5 参照)。代わりに kunai は **option-walk emitter** (`pkg/kunai/codegen/where.go::genOptionLookupLoad` — anchor 経由で abs / slot mode 両対応) という専用 codegen を持つ:
