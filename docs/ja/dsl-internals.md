...
#### `codegen/` (~5,500 行、最大) — IR → `asm.Instructions`
...
9. **`capture.go`** — `headers+N` の長さ algo
10. **`option_demand.go`** — TLV-walk callback が record する dynamic-aux offset の per-(layer, queried-aux) **demand-driven** 割当。`queriedOptions` map が where/capture/predicate を walk して「実際に参照される aux」を集め、`dynamicAuxOffsetSlotBase` から下の領域を per-layer demand size に応じて pack する (eligible aux 数ではなく referenced aux 数で課金)
11. **`callback_lint.go`** — bpf_loop callback の compile-time **branch-count tripwire**。 `assertCallbackComplexity` が conditional + Ja の総数を counter、 `callbackBranchThreshold` を超えたら `ErrNotImplemented` で reject (rationale + 閾値の根拠は同ファイル冒頭コメント)。 「new branch in callback → MAX_DEPTH × scalar-ID inflation」 fingerprint への systemic mitigation
12. **`parser_state.go` / `parser_trail.go` / `parser_select.go` / `parser_loop.go`** — parser-machine codegen の 4 ファイル: `parser_state.go` = 状態 walk root + entry dispatch、 `parser_trail.go` = `pkt.advance` 系 variable trailer (mechanism 1)、 `parser_select.go` = transition select tuple-key resolution (lookahead / bit-slice)、 `parser_loop.go` = bpf_loop callback (multi-state TLV walk + parser self-loop + dynamic aux slot prelude)
13. **`parser_counter.go`** — `extern ParserCounter { ... }` (mechanism 8) の `pc.set` / `pc.decrement` / `pc.is_zero` を bpf_loop callback の prelude / per-iter ops に下ろす。 1-key (`select(pc.is_zero())`) / 2-key (`select(pc.is_zero(), pkt.lookahead<bit<8>>())`) 双方の dispatch を生成
...
- ipv4 のように self-validation (`transition select(version) { 4: skip_options; default: reject; }`) と variable-trailer (`pkt.advance(((bit<32>)(hdr.ihl - 5)) << 5)`) が parser block 内で同居しても、 codegen は state graph を順番に消化するため OK (`validateLayoutExclusivity` は両立を許可)
...
- ABI: R1==index, R2==ctx
- ctx struct (stack 上、4 slot 全て u64): `{offset, scratchStart, scratchEnd, layerEntry}` (各 8 byte)。base offset = `bpfLoopCtxBaseOffset = -208` (10b で `maxArithDepth` 8 → 16 bump に伴い -64 シフト)、各 slot 定数は `bpfloop.go::bpfLoopCtx*Slot`
...
算術: tree walk で R3 に結果を残す契約、stack に左オペランドを退避。**16 段ネストまで** (`maxArithDepth = 16`、 10b で 8 → 16 bump、 stack 再配置で `whereLayerEntrySlotBase = -224` / `dynamicAuxOffsetSlotBase = -280` に連鎖シフト、 `whereLayerEntrySlotCap = 7`)。
...
**`CaptureInfo.MaxCapLen` の解釈**: 0 は「analyser bail / 句なし」両方を含む sentinel で、 **host 側 fallback** を意味する。 xdp-ninja host は `DefaultCapLen = 1500` (libpcap 既定) を埋める。 つまり `capture` 句を書かなければ tcpdump 互換の full packet capture になる。 これは設計上 important: 「`tcp where dport==443` と書いたら payload まで取れる」 という principle of least surprise を満たすため、 `inferMinCapLenFromWhere` の値は `MaxCapLen` には流さず **`FilterMinPrefix` (= in-kernel scratch read sizing) にのみ流す**。 ringbuf 予約を縮めて throughput を上げたい case は `capture headers` 等を明示する。 R32 bench script `benchmark/pipelines/r32_dynamic_scratch.sh` は その明示を行っている。
...
`vocab/p4lite/lexer.go::rejectedKeywords` で **明示的に reject**:
- `action`, `table`, `control`, `apply`

これらが書かれた `.p4` ファイルを読むと `p4lite does not support "..."` エラー。 `extern` は受理されるが、 vocab loader が認識する extern は **`ParserCounter` のみ** (B-5 / mechanism 8)。 他 extern (Hash / Counter / Register / 任意の architecture extern) は parser 段階で本体を opaque-skip し、 loader が silently 無視する。
...
- `extern` (本体ボディ) — match-action / external function call を意味する extern は意味的に未対応。 ただし B-5 で **`ParserCounter` のみ extern 宣言として受理** している (lexer は `extern` を accept、 parser は本体を opaque-skip し、 vocab loader が ParserCounter インスタンス化を認識する)。 通常の P4 architecture extern (`Hash`、 `Counter`、 `Register` 等) は loader が無視する
- `varbit<N>`, header stacks 内宣言 — 動的長 header / オプション解析。今は scratch 512B 内の静的フィールドアクセスのみ
...
§6.1 の wire パターン (A/B/C/D) に対し、vocab 著者が使える declaration mechanism は 8 種類ある。**1 つの protocol が複数 mechanism を併用する**ことも普通 (TCP は mechanism 1 (`pkt.advance` template-A) で固定 trailer を確保しつつ、 その中身は mechanism 7 (TLV options walk) で読む、 など)。
...
**Codegen 経路**: `parse_options → parse_<kind> → parse_options` の cycle を `IsMultiStateLoopEntry` predicate (`pkg/kunai/vocab/parser_machine.go`) が検出し、`emitMultiStateSelfLoop` (`pkg/kunai/codegen/parser_loop.go`) が bpf_loop callback に下ろす。
...
codegen が flag byte を読んで、各 trigger bit が立っていれば対応 LEN だけ R4 を advance する命令列を順に emit する (`codegen.go::emitFlagTriggers`)。parser block を書く必要はない。NAME は uppercase の任意 token で、TRIGGER と LEN を NAME で対応付ける。
...
implementation 詳細は `pkg/kunai/codegen/parser_state.go` の state graph emit ロジックと `parser_loop.go` の bpf_loop callback emit を参照。
